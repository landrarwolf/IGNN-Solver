...
parser.add_argument('--dataset', type=str, default="coraml",  
                    help='Dataset to use.')
...
initfile = os.path.join(thisfolder, 'config/', str(args.labelrate) + 'coraml.ini')
...
def train_f(e, threshold, simple=False):
    ...
    f1_train_micro = accuracy(label_pred[idx_train], labels[idx_train])
    ...
    # loss_val = criterion(label_pred[idx_val], labels[idx_val])
    # f1_val_micro, f1_val_macro = Evaluation(label_pred[idx_test], labels[idx_test])
    loss_test = criterion(label_pred[idx_test], labels[idx_test])
    f1_test_micro = accuracy(label_pred[idx_test], labels[idx_test])
    # acc_test = accuracy(label_pred[idx_test], labels[idx_test])
...
def train_hypsolver(e, threshold, simple=True):
    ...
    # loss_train = criterion(label_pred[idx_train], labels[idx_train])
    # loss_hypsolver = 0.5 * loss_train + loss_hypsolver
    ...
    return f1_test_micro, model_eval_out
...
def solver(ep):
    ...
    for e in range(ep):
        ...
        # for name, param in model.hypsolver.named_parameters():
...
def hypsolver(ep):
    global epoch
    for e in range(ep):
        _,  model_eval_out = train_hypsolver(e, simple=True, threshold=args.threshold)
        # for name, param in model.V.named_parameters():
        #     print(f'Parameter name: {name}, size: {param.size()}, value: {param.data}')
...
def test(threshold=30):
    ...
    label_pred = model_out['label_pred']
    # loss_test = criterion(label_pred[idx_test], labels[idx_test])
    f1_test_micro = accuracy(label_pred[idx_test], labels[idx_test]).cpu().detach().numpy()
...
if args.model == "IGNN_solver" or args.model == "IGNN_ori":
    for ep in range(args.epochs):
        micro = train_f(ep, simple=False, threshold=args.threshold)
        if micro >= micro_max:
